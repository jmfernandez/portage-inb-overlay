...
src_unpack() {
	...
	# Should it use a external dependency on crypto++?
	mkdir "$S/tools/$CRYPTOPPDIR"
	cd "$S/tools/$CRYPTOPPDIR"
	ln -s ../"$CRYPTOPPDIR".zip
	unpack "./$CRYPTOPPDIR".zip

	rm "$CRYPTOPPDIR".zip test.cpp
}

src_configure() {
	# 1.4.4+ : http://synergy-foss.org/pm/issues/3016
	append-ldflags $(no-as-needed)
	epatch "${FILESDIR}"/${P}-nulljobbuildup.patch
...
src_install () {
	...
	if use qt4 ; then
		...
		newicon res/synergy.ico q${PN}.png
		make_desktop_entry ${PN} ${PN/s/S} ${PN} Utility;
	fi
