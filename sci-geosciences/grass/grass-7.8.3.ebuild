...
src_prepare() {
	...
	sed -e "s:\(GRASS_PYTHON=python\).*:\1${pyver}:" -i "${S}/lib/init/grass.sh" || die
	sed -e "s:\(= python\).*:\1${pyver}:" -i "${S}/include/Make/Platform.make.in" || die
...
src_configure() {
	...
	local myeconfargs=(
		...
		--with-proj-includes="${EPREFIX}/usr/include"
