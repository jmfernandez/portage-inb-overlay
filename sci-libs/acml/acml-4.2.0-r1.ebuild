...
inherit eutils toolchain-funcs fortran-2 versionator
...
CDEPEND="ifc? ( dev-lang/ifc )
	gfortran? ( sys-devel/gcc:4.3 )
	!gfortran? ( !ifc? ( sys-devel/gcc:4.3 ) )"
...
pkg_setup() {
	...
	if use test; then
		...
		if  use gfortran &&
			...
			[[ $(gcc-version) != 4.3 ]]
		then
			eerror "You need gfortran-4.3 to test acml"
			eerror "Please use gcc-config to switch gcc version 4.3"
